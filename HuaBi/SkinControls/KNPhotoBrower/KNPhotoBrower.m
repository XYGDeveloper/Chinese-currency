...
- (void)operationBtnIBAction{
    ...
    if(_actionSheetArr.count != 0){ // 如果是自定义的 选项
        ...
        KNActionSheet *actionSheet = [[KNActionSheet alloc] initWithCancelTitle:@"取消" destructiveTitle:@"保存图片" otherTitleArr:nil actionBlock:^(NSInteger buttonIndex) {
...
//        }];
        ...
        KNActionSheet *actionSheet = [[KNActionSheet alloc] initWithCancelTitle:nil destructiveTitle:@"保存图片" otherTitleArr:nil  actionBlock:^(NSInteger buttonIndex) {
            // 让代理知道 是哪个按钮被点击了
            if([weakSelf.delegate respondsToSelector:@selector(photoBrowerRightOperationActionWithIndex:)]){
                [weakSelf.delegate photoBrowerRightOperationActionWithIndex:buttonIndex];
            }
...
#pragma mark - 下载图片
                    ...
                    if(items.url){ // 如果是网络图片
                        EMSDWebImageManager *mgr = [EMSDWebImageManager sharedManager];
                        ...
                        [mgr diskImageExistsForURL:[NSURL URLWithString:items.url] completion:^(BOOL isInCache) {
                            if(!isInCache){
                                ...
                                [[mgr imageCache] queryDiskCacheForKey:items.url done:^(UIImage *image, EMSDImageCacheType cacheType) {
                                    

                                    
                                    if([UIImage isAnimatedGIFData: UIImagePNGRepresentation(image)]){
    ...
    [mgr diskImageExistsForURL:[NSURL URLWithString:items.url] completion:^(BOOL isInCache) {
        if(isInCache){
            if([[[[items.url lastPathComponent] pathExtension] lowercaseString] isEqualToString:@"gif"]){ // gif 图片
                ...
                [[mgr imageCache] imageFromMemoryCacheForKey:items.url];
